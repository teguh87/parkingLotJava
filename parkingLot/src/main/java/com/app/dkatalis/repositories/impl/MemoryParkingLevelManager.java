    ...
    public int parkCar(T vehicle)
    {
        ...
        else
        {
            ...
            if (slotVehicleMap.containsValue(Optional.of(vehicle)))
                return Action.VEHICLE_ALREADY_EXIST;
...
//        for (int slotNumber : slotVehicleMap.keySet()) {
//            Optional<T> vehicle = slotVehicleMap.get(slotNumber);
//            if(vehicle.get().getRegistrationNo().equals(registerNo)) {
//                if(leaveCar(slotNumber)) return slotNumber;
//            }
//        }
        int slot = 0;
        try {
            slot = slotVehicleMap
                    ...
                    .filter(v -> v.getValue()
                            .get()
                            .getRegistrationNo().equals(registerNo))
                    .findAny().get().getKey();
            leaveCar(slot);
        } catch (Exception e) {}
